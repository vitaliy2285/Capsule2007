const { sb, ok, preflight, env } = require('./_supabase');

const TOTAL_CELLS = 20007;
const START_OCCUPIED_FALLBACK = 217;
...
async function countOccupiedCapsules() {
  const url = env('SUPABASE_URL').replace(/\/$/, '') +
    '/rest/v1/capsules?or=(status.eq.published,status.eq.paid_pending_moderation,status.eq.hidden,status.eq.paid_expired_manual_review)&select=cell_number';
...
const handler = async (event) => {
  ...
  try {
    const rows = await withTimeout(sb(
      `/capsules?cell_number=gte.${start}&cell_number=lte.${end}&or=(status.eq.published,status.eq.paid_pending_moderation,status.eq.hidden,status.eq.paid_expired_manual_review)&select=cell_number,nickname,memory_year,message,status,updated_at,is_seed,source&order=cell_number.asc`,
      { method: 'GET' }
    ...
    try {
      ...
      console.warn('[ListCells] Supabase count fallback', { error: countErr.message });
      occupiedTotal = Array.isArray(rows) && rows.length
        ? Math.max(START_OCCUPIED_FALLBACK, rows.length)
        : START_OCCUPIED_FALLBACK;
    }
    ...
    console.warn('[ListCells] fallback', { error: e.message });
    return ok({
      sector,
      start,
      end,
